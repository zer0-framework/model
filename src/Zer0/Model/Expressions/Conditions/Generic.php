...
class Generic extends \Zer0\Model\Expressions\Generic
{
    ...
    protected function parseExpr ($expr)
    {
        preg_replace_callback(
            ...
            function ($match) {
                ...
                else if ($token === self::TOKEN_FIELD) {
                    if ($this->operator === 'OR') {
                ...
                else if ($token === self::TOKEN_FCALL) {
                ...
                else if ($token === self::TOKEN_PLACEHOLDER) {
                    if ($value === '?') {
                ...
                else if ($token === self::TOKEN_CONST_INT) {
                ...
                else if ($token === self::TOKEN_OPERATOR) {
                ...
                else if ($token === self::TOKEN_SUBCLAUSE) {
    ...
    public function delete ($cb = null)
    {
        if ($cb !== null) {
            ...
            foreach ($this->model->storages(true) as $k => $storage) {
                $cj(
                    $k,
                    function ($k, $cj) use ($storage) {
                        $storage->delete(
                            $this,
                            function ($affected) use ($k, $cj) {
                                $cj[$k] = $affected;
                            }
                        );
                    }
    ...
    public function count ($limit = null, $innerJoins = [], $groupBy = null, $cb = null)
    {
        if ($cb !== null) { // If $cb is given, do it asynchronously
            $cj =
                new ComplexJob(
                    function ($cj) use ($cb) { // This callback gets called when all tasks are done
            ...
            $cj->maxConcurrency(1)// Only one task shall be running simultaneously.
               ->more(
                function ($cj) use ($groupBy, $limit, $innerJoins) {
